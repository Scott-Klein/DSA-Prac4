...
class ScottsSmartPlayer : BestFirstPlayer
{
public:
	ScottsSmartPlayer(char ps) : BestFirstPlayer{ ps } 
	{

	}

	virtual void getMove(ticTacToe board, int& x, int& y) {
		cout << "RUNNING VIRTUAL GET MOVE" << endl;
		if (board.isFull())
			return;
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				...
				if (tempBoard.isValidMove(i, j)) {
					...
					Cell c(i, j, tempBoard.improvedHeuristics(playerSymbol));
					moves.push(c);
				}
			}
		}
